...
Le migration `067_kona_ai_codex_esecuzioni.sql` e `068_kona_ai_observer.sql` preparano rispettivamente il registro delle esecuzioni e il dominio server-only dell'Observer (telemetria ripulita, fingerprint, outbox Telegram e checkpoint). Sono state applicate prima sul Supabase staging e, dopo verifica, sul production il 2026-08-11. Il webhook e il cron possono avviare soltanto analisi read-only; patch e test restano nello staging, mentre merge su `main` e deploy production restano fuori dall'automazione.
...
## Registrazione workflow GitHub

GitHub accetta `workflow_dispatch` soltanto per workflow presenti sulla branch predefinita. `main` contiene quindi `.github/workflows/guardian-codex-*.yml`, `guardian-observer-analysis.yml` e `.github/codex/` come control plane dei due ambienti. Le guardie `github.ref` consentono l'analisi production su `main`, analisi e patch staging su `codex/kona-ai-guardian-staging`, e test/proposta di rilascio soltanto da branch `codex/kg-*`. Questi file non sono inclusi nella build Netlify `dist/` e non autorizzano merge o deploy automatici.
