...
| `js/auth.js` | `window.Auth` | `richiediAuth()` guard, `logout()`, `getProfilo()`, `riautentica()` (verifica la password dell'account corrente via Supabase per azioni sensibili; nessuna password operativa nel codice) |
...
- **Conferma azioni sensibili**: **MAI** password operative hardcoded nel frontend. Per chiedere conferma dell'identità corrente usare `await Auth.riautentica({...})`, che verifica via Supabase la password dell'account già loggato. Usata da rimborso manuale e passaggio Apri/Chiudi a KO.
