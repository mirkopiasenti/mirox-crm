...
- `admin-kpi-call-center.js` (GET) — endpoint admin-only che classifica `chiamate` come Consumer e `call_center_lead_outbound_chiamate` come Business outbound, aggrega per giorno e operatrice e restituisce serie annuali per le viste giornaliera, settimane del mese e mensile. `created_at` determina il giorno di lavoro: `originato_da_id IS NULL` vale come nuovo fissato, altrimenti come rischedulato. Presenza ed esiti seguono `data_ora`; le chiusure automatiche per cliente passato in anticipo sono vinte e non annullate, mentre i presentati senza esito sono `Da esitare`. Consolida gli alias tramite `profili.alias_di` e non modifica le tabelle Call Center condivise.
...
**Dashboard**: i reinserimenti sono esclusi sia dal Day by Day sia dai dati mensili/Gare Individuali/Avanzamento Mensile. In `moduli/dashboard_pezzi.html` l'export PNG usa `html2canvas@1.4.1`: Gare Individuali espone un download per ciascuna scheda operatore e cattura soltanto quella scheda; Avanzamento Mensile espone un unico download che cattura insieme le due tabelle del mese selezionato. I pulsanti di download non devono comparire nel PNG. La metrica `TELEFONI CB` di entrambe le dashboard mensili è allineata dalla migration `072` al KPI Customer Base Consumer: conta soltanto `Telefono Incluso`, `dispositivo_associato=true` e `tipo_acquisto` VAR/Finanziamento; Cambi Piano e Caring sono esclusi. Metrica derivata "tasso di rilavorazione = reinserimenti / inserimenti totali" ancora da implementare.
...
| `admin-kpi-vendita-consumer.html` | Reparto KPI, modulo Vendita - Consumer. Tab attive nell'ordine Mobile, Fisso, Customer Base, Luce & Gas, Allarmi, Assicurazioni. Filtri anno/negozio/operatore e tabelle KPI mensili |
| `admin-kpi-vendita-business.html` | Reparto KPI, modulo Vendita - Business. Tab Mobile, Fisso, Luce & Gas, Allarmi e Assicurazioni, limitate a `cluster_cliente='Business'` |
...
- `Chiamate fatte` somma le righe nel giorno di `data_ora` in `Europe/Rome`, mostrando separatamente `chiamate` come Consumer e `call_center_lead_outbound_chiamate` come Business outbound; `Con risposta` esclude soltanto `esito='non_risposto'`.
- `Appuntamenti fissati` usa `appuntamenti.created_at` e conta soltanto `originato_da_id IS NULL`; una riga con `originato_da_id` valorizzato è `Rischedulata`, attribuita al giorno in cui è stato effettuato lo spostamento. Presentati, non presentati, vinti, persi, da esitare e annullati usano invece il giorno previsto in `appuntamenti.data_ora`.
- `Vinti` corrisponde a `esito_finale='vinta'` e include anche `stato='annullato'` con `motivo_modifica` che inizia per `Chiuso automaticamente: cliente passato in anticipo`; queste conversioni anticipate non incrementano `Annullati`. `Persi` usa `esito_finale='persa'`, `Da esitare` richiede `presentato='si'` ed esito nullo, `Non presentati` usa `presentato='no'`, mentre `Annullati` contiene gli altri `stato='annullato'`.
...
- I tassi aggiuntivi sono: risposta = con risposta / chiamate; appuntamento = fissati / chiamate; appuntamento su risposte = fissati / con risposta; presenza = presentati / (presentati + non presentati); chiusura = vinti / (vinti + persi); completamento esiti = (vinti + persi) / (presentati + conversioni anticipate).
...
- `Dettaglio Smartphone` usa il flag del singolo contratto `dispositivo_associato=true`, non la capacità dell'offerta di supportare dispositivi. Nel KPI Mobile Consumer il totale è suddiviso tramite `tipo_acquisto` nelle righe `VAR` e `Finanziati`; le righe FWA Indoor aggiunte alle acquisizioni Mobile restano escluse dal dettaglio Smartphone.
...
- Il tab Customer Base è presente soltanto nel KPI Consumer, tra Fisso e Luce & Gas, e legge `categoria_snapshot='Customer Base'` nel mese di `data_contratto`. `Telefono incluso` richiede `dispositivo_associato=true`, separa `tipo_acquisto='Finanziamento'` e `'VAR'` e mostra la loro somma; i contratti senza dispositivo o tipo acquisto valido non entrano nella tabella. `Cambi piano` conta soltanto `Cambio Piano - TIED` e `Cambio Piano - UNTIED`, con totale. `Caring` separa `CB Caring - FISSO` e `CB Caring - MOBILE` senza aggiungere un totale.
...
- Le pagine Consumer e Business condividono endpoint e JavaScript: il body dichiara `data-kpi-cluster`, il client invia `cluster` e il backend accetta esclusivamente `Consumer` o `Business`. Nessuna aggregazione può mescolare i due cluster; i contratti Customer Base vengono caricati soltanto per `cluster='Consumer'`.
- Il filtro punto vendita accetta `all`, `9001415852` (Legnago) e `9000822241` (Cerea), con default `all`.
- Il filtro operatore è globale per tutte le categorie e offre `Tutti` oppure un singolo profilo canonico; al cambio aggiorna immediatamente ogni tabella senza richiamare l'endpoint. Gli account storici confluiscono nel profilo canonico seguendo `profili.alias_di`; una riga senza operatore resta selezionabile come `Operatore non assegnato`. Se l'operatore scelto non ha dati in una categoria, tutte le metriche di quella categoria valgono zero.
