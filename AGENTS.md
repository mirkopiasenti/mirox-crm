...
- **RPC v2** `vendita_chiudi_eventi_cc_per_pratica_v2(p_anagrafica_id uuid, p_pratica_id uuid, p_giorni_finestra integer DEFAULT 90)` (migration `079`) — quella chiamata oggi dall'action `finalize`. Oltre a quanto faceva la v1: chiude i **non presentati** (`non_presentato_stato='lavorato'` + `esito_finale='vinta'`) e marca `esito_finale='vinta'` gli appuntamenti con `presentato='si'` ed esito ancora aperto **dentro la finestra** (default 90 giorni: non si riscrive il KPI di mesi vecchi). Motivo: un cliente passato e acquisito restava in coda come "non presentato" e la vendita non risultava vinta da nessuna parte (il KPI "Chiusi / vinti" legge `esito_finale`). La **v1 resta invariata** come fallback (`funzioneAssente` su `PGRST202`). Ritorna `{appuntamenti_annullati, chiamate_chiuse, non_presentati_chiusi, appuntamenti_vinti, skipped}`. Best-effort: un errore RPC diventa warning ma non annulla la pratica già finalizzata.
- **RPC v1** `vendita_chiudi_eventi_cc_per_pratica(p_anagrafica_id uuid, p_pratica_id uuid) RETURNS jsonb` — sostituita dalla v2, mantenuta per il fallback. Ritorna `{appuntamenti_annullati, chiamate_chiuse, skipped}`.
