...
La pagina operatore e' una macchina a stati esplicita (`welcome` → `briefing` → `contact` → `outcome` → `followup`/`calendar` → `transition` → `consumer`/`negozio` → `completed`/`error`): una sola schermata visibile alla volta, nessuna scelta manuale fra Consumer/Business/rilavorazioni/calendario. Ogni richiesta server mostra un loading bloccante; gli appuntamenti Business espongono almeno dieci giornate lavorative nell'orizzonte standard, avanzano dopo la sincronizzazione e sono idempotenti per task. `Ricontattare` puo' usare la pianificazione automatica KONA oppure giorno e fascia scelti dall'operatrice, anche nel flusso Consumer. Il briefing dell'INTERA giornata (MATTINA/POMERIGGIO, solo categorie non vuote) e' calcolato server-side da `briefingGiornata` (riusa i candidati, nessuna fonte parallela). Le transizioni avvengono per **famiglia** (conferme/rilavorazioni/Business/campagne/Consumer), mai fra task della stessa famiglia. Il calendario compare solo dopo `Appuntamento`: Business usa Google personale, Consumer usa lo schermo `negozio` che riusa `get_slot_disponibili` + prenotazione del flusso CC (migration additiva `073` per l'esito `appuntamento` in sessione). KONA avvia da solo la sessione Consumer dal piano (`avvia_consumer`), leggendo `consumer` e il legacy Telegram `categoria_sessione`; se la registrazione dell'esito fallisce, la prenotazione negozio appena creata viene rimossa. `Avvia`/`Avvia chiamate`/`Prossimo` sono idempotenti e il refresh riprende il task attivo. Nessuno script telefonico: l'IA puo' solo contestare lo skip "Altro" una volta.
