...
- **Modifica contratto in Verifica Contratti** (dal 2026-07-20): il popup `moduli/verifica_contratti.html` (funzioni `salvaModifiche` / `confermaVerifica`) ricalcola i punteggi dal catalogo **solo se `offerta_id` o `opzione_id` sono stati effettivamente cambiati** rispetto al valore precedente del contratto. Se l'utente non li tocca, viene preservato lo snapshot storico originale (i punteggi non vengono rifatti anche se l'admin ha modificato il catalogo dopo la creazione). Il ricalcolo è granulare per campo: cambio solo di offerta → aggiorna i 2 punteggi offerta e lascia gli opzione, e viceversa. Impostare `opzione_id` a null azzera i due punteggi opzione. La cache dei cataloghi (`window.__currentVerificaCataloghi.offerte/.opzioni`) include `punteggio_gara` + `punteggio_extra_gara` per il ricalcolo lato client.
- **Prima del 2026-07-20**: il popup di Verifica Contratti aggiornava solo `opzione_id` / `offerta_id` senza toccare i punteggi. Se un operatore aggiungeva un'opzione a un contratto esistente, `punteggio_gara_opzione` restava a 0 e la Gara Individuale non contava il punto. Bug fixato dal 2026-07-20.
