...
1. **Fonti di verità condivise**: `README.md` (utente-facing) + `AGENTS.md` (fonte primaria Codex) + `CLAUDE.md` (questo file, compatibilità storica) + `database/README.md` (schema). Ogni modifica va riflessa nei file pertinenti **nella stessa sessione** in cui si tocca il codice.
...
Tutte le functions usano `SUPABASE_SERVICE_ROLE_KEY` e bypassano le RLS. Per questo motivo **TUTTE le functions tranne i due cron Netlify, `public-prenota` e `guardian-telegram-webhook`** richiedono `Authorization: Bearer <jwt>` valido (validato via `_lib/require-auth.js`). Il webhook Guardian e' protetto da secret token Telegram e allowlist rigida del `chat_id` di Mirko. `admin-vendita-config`, `admin-kpi-vendita-consumer`, `gestisci-controllo-fissi`, `elimina-vendita-contratto`, le action manuali di `gestisci-controllo-lg` e le action sensibili di `gestisci-operazioni-post-vendita` richiedono ulteriore check `ruolo='admin'`. Il client deve usare `MiroxApi.fetch()` o aggiungere l'header manualmente. 26 functions + 9 lib condivise, inclusi `_lib/kona-ai-guardian.js` e `_lib/telegram.js`:
...
- `guardian-incidents.js` (GET/POST action-based) — raccolta autenticata KONA AI Guardian. Gli operatori accedono solo ai propri incidenti; gli admin possono elencarli tutti. Tabelle server-only e identita' derivata dal JWT.
- `guardian-telegram-webhook.js` (POST) — eccezione webhook protetta da secret token + owner `chat_id`. Gestisce **solo testo** (i vocali non sono piu' trascritti: il bot risponde chiedendo di scrivere il messaggio), conversazione, analisi Guardian approvata e archiviazione auditata. Nella prima versione non accede al repository e non esegue Codex.
...
- `_lib/kona-ai-guardian.js` — prompt e output strutturati, fallback raccolta, analisi proprietario, codici `KG-*` e notifica Telegram.
- `_lib/telegram.js` — client Telegram privato del Guardian, **solo testo** (`sendTelegramMessage`, `answerCallbackQuery`, `telegramRequest`). La trascrizione dei vocali è stata rimossa il 2026-09-14: nessun download audio, nessuna chiamata a OpenAI Audio Transcriptions, env `OPENAI_TRANSCRIBE_MODEL` non più usata.
- Lib KONA Call Director (vedi sezione dedicata): `_lib/kona-cd-openai.js` (Responses API + ricerca web), `_lib/kona-cd-deepseek.js` (chat-completions JSON, thinking disattivata), `_lib/kona-cd-ai.js` (dispatcher per attività), `_lib/kona-cd-assistente.js` (intento Telegram + regola delle conferme), `_lib/kona-cd-agenda.js` (costruzione guidata della giornata: finestre, orari, blocchi, tempo residuo), `_lib/kona-cd-budget.js` (riserve atomiche, RPC `v2` con fallback), oltre a engine, arricchimento, report, notifiche, google, retention, config, telegram, util, time, scoring, distances, conferme, crypto.
...
- `profili` — utenti CRM, `ruolo` IN ('admin','operatore'), `pagine_accessibili` jsonb per ACL Call Center, `in_gara` bool (dal 2026-07-09, migration 038) per includere l'operatore nella tab "Gare Individuali" del Dashboard Pezzi, `alias_di` uuid (dal 2026-07-09, migration 040) self-FK opzionale per unificare due account della stessa persona: se valorizzato, il profilo e' un alias del profilo canonico indicato e ogni INSERT/UPDATE su tabelle con `operatore_id` viene redirezionato al canonico tramite trigger `trg_alias_operatore`. Il backfill usa `applica_alias_backfill_v2(uuid)` (migration `053`), SECURITY DEFINER con controllo admin; la v1 viene chiusa al client dalla migration post-deploy `054`.
- `anagrafica` — cliente unificato, `cf_piva` UNIQUE, `cluster` operativo condiviso (`Consumer`/`Business`; i passaporti cluster vendita `Turista` vengono salvati qui come `Consumer`). Colonna `email` (text, NULL ammesso a livello DB; obbligatoria lato wizard solo per `Consumer`/`Business`). RPC `cerca_o_crea_anagrafica(p_..., p_email)` UPSERT. Migrations `057` + `059`: consolidate 8 duplicazioni certe da identificativi fiscali/import errati e dal doppione tecnico `TEST`/`test`, spostando tutte le FK e conservando ogni record eliminato in `vendita_log_modifiche`; le omonimie con CF/P.IVA validi distinti restano separate.
...
- **RPC v2** `vendita_chiudi_eventi_cc_per_pratica_v2(p_anagrafica_id uuid, p_pratica_id uuid, p_giorni_finestra integer DEFAULT 90)` (migration `079`) — quella chiamata oggi dall'action `finalize`: fa tutto quello che faceva la v1 e in piu' (a) porta i **non presentati** a `non_presentato_stato='lavorato'` + `esito_finale='vinta'` e (b) marca `esito_finale='vinta'` gli appuntamenti con `presentato='si'` ed esito ancora aperto **dentro la finestra** (default 90 giorni, per non riscrivere il KPI di mesi vecchi). Motivo: un cliente passato e acquisito restava in coda come "non presentato" e la vendita non risultava vinta da nessuna parte (il KPI Call Center legge "Chiusi / vinti" da `esito_finale`). La **v1 resta invariata** e viene usata come fallback finche' la migration non e' applicata (`funzioneAssente` sul codice `PGRST202`). Ritorna `{appuntamenti_annullati, chiamate_chiuse, non_presentati_chiusi, appuntamenti_vinti, skipped}`. Best-effort: un errore RPC diventa warning ma non annulla la pratica finalizzata.
- **RPC v1** `vendita_chiudi_eventi_cc_per_pratica(p_anagrafica_id uuid, p_pratica_id uuid) RETURNS jsonb` (migration `028`) — sostituita dalla v2, mantenuta per il fallback. Ritorna `{appuntamenti_annullati, chiamate_chiuse, skipped}`.
- Logica della v1 (invariata nella v2):
  - **Annulla** `appuntamenti` con `stato='confermato'` AND `presentato IS NULL` AND `data_ora >= ieri` → `stato='annullato'` + `motivo_modifica='Chiuso automaticamente: cliente passato in anticipo, pratica vendita <uuid>'`. Lascia stare `presentato='si'` (vanno esitati in "Esiti Appuntamenti") e `presentato='no'` (nella v1 restavano per "Rilavorazione → Non Presentati"; **la v2 li chiude**)
...
Al 2026-07-02 il konahub e' stato dismesso e `moduli/segnalazioni.html` e' stata **modernizzata**: include `js/config.js` + `js/auth.js`, chiama `Auth.richiediAuth()` al boot, aliasa `const sb = window.db` (client persistSession della sessione Mirox). La migration 036 revoca le 5 policy `anon` residue; dalla migration post-deploy `054` anche gli upload authenticated passano da `upload-documento-modulo` e non esiste più una policy INSERT browser sul bucket.
...
- **Creazione contratto**: il frontend NON manda i punteggi. Il backend `crea-vendita-pratica-carrello.js` li legge dal catalogo (`vendita_offerte.punteggio_gara/punteggio_extra_gara` + `vendita_opzioni.punteggio_gara/punteggio_extra_gara`) e li scrive come **snapshot** nelle 4 colonne `vendita_contratti.punteggio_gara_offerta`, `punteggio_extra_gara_offerta`, `punteggio_gara_opzione`, `punteggio_extra_gara_opzione`.
- **Guardie pre/post INSERT (dal 2026-07-26)**: il backend fa una seconda query indipendente sul catalogo, rifiuta tutti e quattro i componenti nulli/vuoti/non numerici tramite `_lib/score-integrity.js`, quindi confronta i componenti e i totali della riga realmente restituita dal DB con quelli attesi. Una divergenza fa fallire la creazione e attiva il rollback della pratica ancora in bozza.
- **Modifica contratto in Verifica Contratti**: il client non invia mai campi punteggio e il backend li deriva dal catalogo **solo se `offerta_id` o `opzione_id` cambiano**. Senza cambio conserva i quattro componenti già presenti; con cambio rifiuta cataloghi nulli/non numerici. Dopo l'UPDATE verifica componenti, totali e colonne legacy della riga restituita.
- **Bug 25/07/2026** (fixato): la SELECT di `caricaContratti()` in `verifica_contratti.html` NON caricava i 4 campi `punteggio_gara_offerta` / `punteggio_extra_gara_offerta` / `punteggio_gara_opzione` / `punteggio_extra_gara_opzione`. Quindi `c.punteggio_gara_offerta = undefined`, `numOr(undefined, 0) = 0`, e ogni "Conferma verifica" senza cambio offerta **sovrascriveva a 0** gli snapshot corretti. Impatto: 25+ contratti luglio azzerati (poi rifixati manualmente via UPDATE SQL da catalogo). Fix: omettere del tutto i 4 campi dal patch UPDATE se offerta/opzione non sono state cambiate. Il trigger DB ricalcola i totali dai valori NEW (che sono i valori DB attuali se non passati).
- **Prima del 2026-07-20**: il popup di Verifica Contratti aggiornava solo `opzione_id` / `offerta_id` senza toccare i punteggi. Se un operatore aggiungeva un'opzione a un contratto esistente, `punteggio_gara_opzione` restava a 0 e la Gara Individuale non contava il punto. Bug fixato dal 2026-07-20 (che ha introdotto il bug 25/07/2026, fixato definitivamente col patch selettivo sopra).
- **Trigger** `vendita_calcola_punteggio_totale` (BEFORE INSERT/UPDATE): somma i 4 punteggi (offerta + opzione) e li scrive nelle colonne totale (`punteggio_gara_totale`, `punteggio_extra_gara_totale`). NON legge dal catalogo — si limita a sommare i valori già presenti in NEW.
- **Vincoli e audit DB** (migration `058`): tre CHECK validati impediscono la persistenza di totali gara/extra o colonne legacy incoerenti. Il trigger AFTER `trg_vendita_contratti_audit_punteggi` registra da ora ogni INSERT e ogni variazione dei quattro componenti in `vendita_log_modifiche`.
...
- `collectContractDraft()` salva il valore nel carrello e `buildInvioPayload()` deve propagarlo esplicitamente al backend; non rimuovere questo pass-through, altrimenti ogni contratto Cerea ricade sul default Legnago
...
2. La funzione fa due query: prima recupera i contratti `vendita_contratti` del cliente per quella categoria nel **mese solare corrente** (timezone Europe/Rome, estremi calcolati con offset distinti per gestire correttamente i cambi ora legale di marzo/ottobre, `data_contratto >= inizio_mese AND < inizio_mese_successivo`, `stato_inserimento='inserimento'`, esclude catene), poi recupera dalla tabella post-vendita appropriata gli stati che fanno scattare il popup. Un contratto del mese precedente è sempre un inserimento nuovo
...
- Se `'reinserimento'`: valida `reinserimento_di_contratto_id` come UUID, verifica **stessa anagrafica**, **stessa categoria**, **stesso mese solare Europe/Rome** e ricontrolla lo stato ammesso nella tabella post-vendita pertinente (errori 400 altrimenti)
...
- **Stati ammessi** (CHECK `pvcf_stato_chk`): `Da completare`, `In Attivazione`, `Attivo`, `KO`. Transizioni: `Da completare` → `In Attivazione` (via bottone "Compilazione Completata") → `Attivo` (richiede `data_attivazione` effettiva) | `KO` (azzera `attivazione_prevista`, opzionale `motivo_ko`). `KO` è normalmente terminale, ma un admin può correggere un esito inserito per errore e riportare sia `KO` sia `Attivo` a `In Attivazione` dopo conferma esplicita. Il ripristino aggiorna l'audit, azzera `data_attivazione` e rimuove `motivo_ko`.
...
- soltanto `TELEGRAM_GUARDIAN_OWNER_CHAT_ID` puo' conversare via bot, e **solo con testo**: i vocali non vengono più trascritti (rimossi il 2026-09-14) e ricevono la richiesta di scrivere il messaggio;
...
- `moduli/dashboard_pezzi.html`: layout compatto con tabella fissata a 622px.
- `moduli/upload-contratti-vendita.html`: redirect alla dashboard dopo submit; gestione dedicata del cluster `Turista`; popup OCR e orario locale restano senza invio email tecnico.
- `moduli/verifica_contratti.html`: filtro Giorno e convergenza Fisso nel dettaglio.
- `js/mirox-upload.js`: anteprima PDF centralizzata per i moduli operativi.
...
Modulo **server-only** per il Call Center: un contatto alla volta, conferme Business, arricchimento notturno e pianificazione Telegram.

**Hardening post-audit (2026-09-01)** — ~40 difetti corretti. Le migration `database/076_kona_call_director_hardening.sql` e `database/077_kona_call_director_rpc_hardening.sql` e lo script `database/staging/006_kona_call_director_negozio_dipendenze.sql` sono stati **applicati il 2026-09-14 al solo Supabase test `yyorullxmdxhnunsfwwa`** (production non toccata), con verifica automatica superata su tutti i controlli. Sintesi: whitelist delle colonne scrivibili nell'arricchimento (`telefono_extra` non esiste su `call_center_lead_outbound` e faceva fallire ogni arricchimento con retry pagati); `codice_fiscale` fuori dai campi arricchibili; applicazione solo con almeno una fonte web reale; nessun dato personale nel prompt con ricerca web attiva; `logUsage` non lanciante con rilascio garantito della riserva e costo registrato anche su output invalido; `0` rispettato come valore esplicito nelle manopole di costo; claim atomico del task; chiusura sempre della sorgente outbound; blacklist/esclusioni paginate; un candidato "avvelenato" non blocca piu' la coda; `parseHHmm` ritorna `null` sui valori vuoti; lease `NULL` scaduto; appuntamenti senza CF gestiti; anagrafica Consumer non azzerata sui campi facoltativi; pausa+refresh recuperabile; piano approvato non sovrascrivibile; failover manuale solo con errore AI registrato dal backend; report non troncato; notifiche Telegram in testo semplice con recupero dei lease `in_invio` e rispetto di `notifiche_immediate`; retention limitata agli stati terminali; rotazione token Google senza perdere `collegato_da` e `collegato` basato sulla decifrabilita' reale; sync Google fallita senza cancellare la riga di riconciliazione; pagine CC con `Auth.richiediAccesso` e `CcHeader.render` atteso; redirect di sessione scaduta corretto; risposta non-JSON non piu' scambiata per successo; 409 con ricarica della scheda; prenotazione negozio atomica via `kona_cd_prenota_negozio_v1` con fallback. `D7` (rilavorazione Consumer) e' stato verificato come **falso positivo**: il trigger canonico `calcola_ricontatto_non_risposto` classifica gia' tutti gli esiti prodotti da `mappaEsitoConsumer`.

**Secondo giro (2026-09-01; applicato il 2026-09-14 al solo staging)** — migration `database/077_kona_call_director_rpc_hardening.sql`. RPC `kona_cd_*` ricreate a firma invariata: `kona_cd_prenota_slot_v1` serializza su chiave **globale** per giornata e controlla i conflitti su tutti gli appuntamenti (il calendario Google e' unico: prima due operatrici potevano prenotare lo stesso istante); `kona_cd_reserve_budget_v1` applica l'idempotenza solo a prenotazioni attive, confronta la chiave troncata come l'INSERT e impone il **tetto orario atomico** dentro la stessa transazione; `kona_cd_correggi_esito_v1` legge il ruolo admin da `profili` invece del booleano del chiamante e valida il massimo di 500 caratteri. Lato codice: compensazione della chiamata canonica se la chiusura della sorgente fallisce (standard e outbound); deduplica `update_id` di Telegram con claim atomico e unico punto di scrittura dello stato conversazione; "Chiamate di oggi" limitata all'operatrice per i non admin; rimossi `consumaRiserva`, le funzioni conferme divergenti e il ramo `telefono_fisso` inesistente; `ultimo_task_at` ora scritto alla materializzazione. **Trade-off accettato**: `kona_call_director_correzioni_esito` resta append-only con FK `ON DELETE RESTRICT`, quindi dopo la prima correzione chiamata e profilo collegati non sono piu' eliminabili: l'integrita' dell'audit prevale sulla cancellabilita' e un rollback strutturale richiede una migration dedicata con approvazione esplicita.

Il Supabase test dedicato `Mirox CRM - Test KONA Call Director` (`yyorullxmdxhnunsfwwa`, `eu-west-3`) contiene bootstrap minimo senza dati production, migration `072`-`075` e seed fail-closed. Il dataset sintetico `staging/004` e' stato bonificato con `database/staging/005_kona_call_director_preparazione_isabella_20260831.sql`: per lunedi' 31/08 resta un solo piano approvato Consumer manuale mattina/pomeriggio, senza categorie Business o arricchimenti automatici; contatti, chiamate, task e appuntamenti partono da zero. Dal 2026-08-29 il sito `mirox-kona-call-director-test.netlify.app` esegue il collaudo controllato con entrambi gli interruttori env e toggle globale a `true`, un solo profilo test abilitato, modalità osservazione `true` e Google Calendar collegato con sync `ok`. I dati reali eventualmente usati nella prova restano esclusivamente nello staging e vanno rimossi al termine. Gli esiti usano `task_id` per rendere idempotente un retry post-salvataggio; la cancellazione Google tratta 404/410 come evento gia' assente. Production e' invariata.

**Assistente Telegram in linguaggio naturale (2026-09-14)** — il bot KONA Call Director non e' piu' un elenco rigido di comandi: si scrive in italiano e il messaggio viene interpretato da **DeepSeek V4.1 Flash** (modello `deepseek-flash`, endpoint OpenAI-compatibile `/chat/completions`, JSON output con thinking disattivata). Architettura a due provider, scelta da `kona_call_director_config.provider_per_attivita` fuso con i default di codice (`{telegram, piano, analisi}` su DeepSeek), mai dal codice:
- `_lib/kona-cd-ai.js` e' il dispatcher: `aiStructured({activity})` instrada su `_lib/kona-cd-openai.js` o `_lib/kona-cd-deepseek.js`. Una capacita' che il provider non ha **non viene mai degradata in silenzio**: chiedere `webSearch: true` a DeepSeek restituisce `provider_non_supporta_web_search`. Su OpenAI restano `arricchimento` (ricerca web) e `altro` (valutazione degli skip).
- **Piano giornaliero e analisi della giornata girano su DeepSeek** (`kona-call-director-plan.js` e `kona-cd-report.js` usano `aiStructured`). Sono attivita' `piano` e `analisi`: restano fuori dalla riserva Telegram e ricadono nella riserva "dialogo". `deepseekStructured` applica il tetto orario dei messaggi SOLO all'attivita' `telegram`.
- Poiche' DeepSeek non impone lo schema (`json_object`, non `json_schema` strict), `validateStructured` e' ora **ricorsivo** (oggetti e array annidati) e il prompt di sistema include un **esempio di json costruito dallo schema** (`esempioDaSchema`): un output fuori forma viene rifiutato e il chiamante degrada al fallback deterministico invece di persistere dati storti.
- `_lib/kona-cd-assistente.js` classifica l'intento (`stato`, `report`, `piano`, `categorie`, `approva_piano`, `sospendi`, `riattiva`, `telefoni_omaggio`, `direttiva`, `conferma`, `annulla`, `altro`) e **propone**, non esegue.
- **Regola delle azioni delicate**: le letture rispondono subito, le azioni che cambiano stato (`sospendi`, `riattiva`, `approva_piano`, `telefoni_omaggio`, `direttiva`) vengono solo riproposte in chiaro con pulsanti "Si/No" e attesa massima 30 minuti. Vale anche per i comandi con la barra (`/sospendi` chiede conferma). `si`/`no` sono riconosciuti con regex, senza chiamare l'IA.
- **Il giorno non si sceglie di nascosto** (`giornoDaTesto`/`giornoRichiesto`/`etichettaGiorno`): "oggi"/"domani" scritti da Mirko vincono su qualunque deduzione del modello, altrimenti il bot CHIEDE per quale giorno (`chiediGiorno`, stato `in_attesa_giorno` che si completa al messaggio successivo) e non applica alcun default automatico. La conferma mostra `oggi 14/09/2026`. Prima esisteva `esito.argomenti.data = domani` come default: una richiesta "di oggi" finiva sul piano di domani.
- **Le categorie di una direttiva sono quelle dei CONTATTI** ("Ristorazione", "Negozi"), non le offerte ("fissi"): la corrispondenza vive in `categoriaCorrisponde` (`_lib/kona-cd-engine.js`, una sola implementazione usata dal filtro candidati e dall'avviso) e se nessun contatto corrisponde il bot lo dice con l'elenco dei nomi disponibili (`avvisoCategorie`).
- **Non esiste una lista fissa di categorie Business**: sono i settori dei contatti in `call_center_lead_outbound`. `categorieDisponibili` + `etichettaCategorie` (webhook Telegram, esposti in `_test`) leggono l'elenco reale con i conteggi; `/categorie` e la domanda "quali categorie posso scegliere?" lo stampano. Le sole categorie fisse sono le due modalita' Consumer (`telefoni_omaggio`, `fibra_fwa`). Non reintrodurre elenchi di esempio inventati ("Bar, negozi, officine"): hanno gia' fatto scrivere direttive con nomi inesistenti.
- **La modalita' Consumer e' una scelta esplicita**: l'assistente la dichiara in `modalita_consumer` (telefoni_omaggio/fibra_fwa) e `normalizzaArgomenti` la accetta solo se prevista. Mai dedurla dalle parole della nota: "fibra" e' anche il nome di un'offerta e riconfigurava la giornata da sola (bug del collaudo). La vecchia `categoriaDaTesto` e' stata rimossa.
- **Agenda guidata** (`/agenda` + azione `agenda`, `_lib/kona-cd-agenda.js`): il bot propone le tre attivita' (lead aziendali, liste Fibra/FWA, telefoni omaggio), fa scegliere le categorie reali per i lead, chiede l'orario, e finche' resta tempo richiede cosa mettere nel tempo residuo. Modulo puro e testato: `finestreGiorno` (da `orario_mattina`/`orario_pomeriggio`), `buchiResidui`/`minutiResidui`, `parseIntervallo` ("15:30-17:00", "90 minuti", "17.01 - 19.00", "mezz'ora"), `componiBlocco` (mai fuori finestra ne' sovrapposto), `domandaOpzioni`/`tastieraOpzioni`. Le due liste Consumer sono una modalita' al giorno. Nessuna scrittura sul piano prima della conferma finale (`applicaAgenda` → `scriviPianoDirettiva`, l'unico punto di scrittura condiviso con la direttiva libera). Le ore sono l'intenzione della giornata: il motore lavora per priorita' dentro le finestre, non con un timer.
- **Il briefing dell'operatrice mostra le categorie approvate** e avvisa quando nessun contatto corrisponde: un piano che non puo' partire si vede subito.
- **Privacy**: al modello che interpreta arrivano solo aggregati (`contestoAssistente`: budget, conteggi, zone); l'interprete non legge alcuna tabella anagrafica. I vocali **non** vengono trascritti: nessun audio inviato a provider (rimossi `downloadTelegramFile`/`transcribeVoice` da `_lib/telegram.js`, che ora e' solo testo).
- **Costi**: riserva dedicata `riserva_telegram_eur` = **10 EUR/mese** e tetto `max_messaggi_telegram_ora` = 60. La spesa e' registrata in `kona_call_director_budget_log` con attivita' `telegram`, quindi separata da quella OpenAI; il tetto orario Telegram e' distinto da `max_chiamate_openai_ora`.
- Migration `database/078_kona_call_director_telegram_ai.sql` (RPC **`kona_cd_reserve_budget_v2`**, nome nuovo: la v1 resta invariata): **applicata dall'utente al solo Supabase test il 2026-09-14**. Il codice ha comunque il fallback sulla v1 con verifica del tetto Telegram in JS.
- Env nuove: `KONA_CALL_DIRECTOR_DEEPSEEK_API_KEY` (obbligatoria per l'assistente) e `KONA_CALL_DIRECTOR_DEEPSEEK_MODEL` (opzionale). Le env di Netlify entrano nelle function solo con un NUOVO deploy: dopo averle aggiunte serve un redeploy. Il trasferimento del testo e degli aggregati a DeepSeek e' extra-UE e va citato nell'informativa privacy.
...
2. ricontatti programmati; 3. auto non risposti; 4. appuntamenti non presentati; 5. "Passa a Cerea"; 6. "Passa in negozio"; 7. lead outbound aziendali, **solo dentro una fascia "Lead Outbound Aziendali"** e solo con categorie esplicitamente approvate nel piano. Le priorita' 1-6 valgono sempre e vincono anche dentro le fasce manuali; le vecchie priorita' 7 (campagne urgenti `pinned`) e 8 (nuovi lead Business sempre proposti) sono state rimosse. La fascia in corso viene da `contenuto.agenda_blocchi` del piano (scritto dall'agenda guidata Telegram) e, in mancanza, dalla `programmazione_base` di configurazione (09:00-10:30 aziendali, 10:31-12:30 Fisso, 15:30-17:00 aziendali, 17:01-19:00 Fisso); fuori dalle fasce non si propongono lead. La scheda mostra l'indirizzo canonico del cliente/azienda. Non presentati e passaggi riproducono il bivio manuale `Presentato`/`Ricontatta`; solo il ricontatto apre gli esiti e un eventuale appuntamento usa il calendario negozio condiviso.
...
Routing per profilo (server-side via `kona-call-director-route` + `js/cc-header.js`): KONA disattivato → tutti usano il manuale; KONA attivo + profilo non abilitato → manuale; KONA attivo + operatrice non-admin abilitata → **solo KONA** (nessuna tab manuale, accesso diretto a una pagina manuale reindirizza a `kona-call-director.html`); admin → manuale + tab `KONA CD` + pannello admin. La tab `KONA CD` in `cc-header.js` e' `adminOnly`, ma dentro `kona-call-director.html` anche l'admin vede sempre la shell minima senza alcuna tab manuale. **Una sola fonte di verita'**: gli esiti Consumer scrivono in `chiamate` (via `registraChiamataConsumerCanonica`) oltre che nell'audit di sessione; gli appuntamenti Consumer scrivono in `appuntamenti` + `chiamate` (esito `appuntamento`). Ricerca numero, storico e correzione esito in giornata passano da `kona-call-director-operator`; l'audit e' append-only. Un guasto AI allowlisted abilita per 30 minuti il solo profilo al manuale e accoda un avviso Telegram senza PII. Le tabelle `kona_call_director_*` restano solo orchestrazione/audit.

### Esperienza operatore (macchina a stati)

La pagina operatore e' una macchina a stati esplicita (`welcome` → `briefing` → `contact` → `outcome` → `followup`/`calendar` → `transition` → `manuale`/`consumer`/`negozio` → `completed`/`error`): una sola schermata visibile alla volta, nessuna scelta manuale fra Consumer/Business/rilavorazioni/calendario. Ogni richiesta server mostra un loading bloccante; gli appuntamenti Business espongono almeno dieci giornate lavorative nell'orizzonte standard, avanzano dopo la sincronizzazione e sono idempotenti per task. `Ricontattare` puo' usare la pianificazione automatica KONA oppure giorno e fascia scelti dall'operatrice, anche nel flusso Consumer. Il briefing (attivita' in corso, lead aziendali pronti, coda MATTINA/POMERIGGIO, solo categorie non vuote) e' calcolato server-side da `briefingGiornata` (riusa i candidati, nessuna fonte parallela). Le transizioni avvengono per **famiglia** (conferme/rilavorazioni/Business/Consumer), mai fra task della stessa famiglia. Il calendario compare solo dopo `Appuntamento`: Business usa Google personale, Consumer usa lo schermo `negozio` che riusa `get_slot_disponibili` + prenotazione del flusso CC (migration additiva `073` per l'esito `appuntamento` in sessione). Nella fascia manuale (schermo `manuale`) KONA non propone contatti: mostra attivita' in corso, contatore `chiamate_fascia` della sessione e il pulsante `Registra chiamata` (action `registra_chiamata_manuale`, categoria decisa dalla programmazione); `Aggiorna fascia` rilegge lo stato e rimette l'operatrice sulla fase giusta. Fuori dalle fasce manuali KONA avvia da solo la sessione Consumer della fascia (`avvia_consumer`, modalita' dalla fascia in corso con fallback su `consumer`/`categoria_sessione`); se la registrazione dell'esito fallisce, la prenotazione negozio appena creata viene rimossa. `Avvia`/`Avvia chiamate`/`Prossimo` sono idempotenti e il refresh riprende il task attivo. Nessuno script telefonico: l'IA puo' solo contestare lo skip "Altro" una volta.
...
- **Identità server-side**: campi audit/ownership come `operatore_id`, `uploaded_by`, `created_by` non sono fonte di verità se arrivano dal client; derivarli da `auth.profilo`/`auth.user` e validare le relazioni fra gli UUID ricevuti.
- **Email**: via `MiroxMailer.send({to, template, vars})` → endpoint `mirox-send-email`. Mai SMTP diretto dal client.
- **Segnalazioni tecniche**: non inviare email automatiche e non agganciare handler globali. Gli operatori usano `moduli/segnala-problema.html`; future fonti automatiche devono creare/deduplicare incidenti Guardian lato server. I popup locali restano responsabili del feedback immediato all'utente.
...
- **Test**: prima di chiudere un task eseguire `npm test`. La suite `node:test` in `/tests` copre regressioni prioritarie, sintassi di JS/script HTML inline e link locali.
