...
**Assistente Telegram in linguaggio naturale (2026-09-14)** — il bot KONA Call Director non e' piu' un elenco rigido di comandi: si scrive in italiano e il messaggio viene interpretato da **DeepSeek V4.1 Flash** (modello `deepseek-flash`, endpoint OpenAI-compatibile `/chat/completions`, JSON output con thinking disattivata). Architettura a due provider, scelta da `kona_call_director_config.provider_per_attivita` (default `{"telegram":"deepseek"}`), mai dal codice:
- `_lib/kona-cd-ai.js` e' il dispatcher: `aiStructured({activity})` instrada su `_lib/kona-cd-openai.js` o `_lib/kona-cd-deepseek.js`. Una capacita' che il provider non ha **non viene mai degradata in silenzio**: chiedere `webSearch: true` a DeepSeek restituisce `provider_non_supporta_web_search`. L'arricchimento con ricerca web resta quindi su OpenAI.
...
- Migration `database/078_kona_call_director_telegram_ai.sql` (RPC **`kona_cd_reserve_budget_v2`**, nome nuovo: la v1 resta invariata): **scritta ma non ancora applicata**. Finche' non lo e', `tryReserveBudget` fa fallback sulla v1 e verifica il tetto Telegram in JS (non atomico, scarto massimo = costo di un messaggio).
- Env nuove: `KONA_CALL_DIRECTOR_DEEPSEEK_API_KEY` (obbligatoria per l'assistente) e `KONA_CALL_DIRECTOR_DEEPSEEK_MODEL` (opzionale). Il trasferimento del testo a DeepSeek e' extra-UE e va citato nell'informativa privacy.
