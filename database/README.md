...
I 39 file `.sql` in questa cartella sono **migrazioni storiche parziali** applicate manualmente nel SQL Editor di Supabase (o via `.bin/supabase db query --linked --file ...`) durante lo sviluppo.
...
| `037_consensi_privacy_v2.sql` | Sistema consensi privacy v2 (attivo dal 2026-07-02). Crea `privacy_policy_versions` (versioni informativa, indice unico parziale garantisce una sola attiva), `vendita_consensi_privacy_v2` (marketing granulare 3 canali `email`/`whatsapp`/`phone_operator` — no SMS come da testo legale approvato — `main_phone` + `otp_phone` separati con `otp_phone_motivazione` obbligatoria via CHECK se differiscono, `consent_uuid` pubblico, `document_hash`/`pdf_hash` SHA256, `presa_visione_at` legata a `informativa_version_id`, `marketing_valido_fino_al` a 24 mesi F5, snapshot anagrafica jsonb), `vendita_consensi_privacy_audit` (append-only con trigger `no_update`/`no_delete`). CHECK di coerenza stato/pdf/motivazione. Indici dedupe presa visione + scadenza marketing + cleanup pending. RLS SELECT authenticated su tutte e 3. Bucket `consensi-privacy` riusato dalla 034. Vedi "Sistema consensi privacy GDPR v2" in `../CLAUDE.md`. |
