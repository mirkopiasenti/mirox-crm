...
| `030_rls_segnalazioni_authenticated.sql` | Chiude la tabella `segnalazioni` a `authenticated` (prima anon). Poi parzialmente rilassata in 032 per il konahub legacy. |
...
- **2026-07-02**: modifiche solo frontend/mailer, nessun cambio schema Supabase. Aggiornati layout `dashboard_pezzi` con larghezze fisse compatte per colonne operatori/offerte, redirect post-invio di `upload-contratti-vendita`, favicon mancanti e link CTA delle email di comunicazione verso `https://www.mirox-crm.it`.
