...
Modulo che coordina la giornata dell'operatore autorizzato del Call Center: un contatto alla volta, priorita' deterministiche, arricchimento Business da fonti pubbliche, appuntamenti per giorno/zona, Google Calendar e pianificazione con Mirko via bot Telegram separato. **Stato verificato**: codice completo e Supabase test dedicato `Mirox CRM - Test KONA Call Director` (`yyorullxmdxhnunsfwwa`, `eu-west-3`) pronto con bootstrap minimo, migration `072`, 23 tabelle KONA, cinque RPC e nessun dato production. Il sito Netlify isolato [`mirox-kona-call-director-test.netlify.app`](https://mirox-kona-call-director-test.netlify.app) e' collegato alla branch `kona-call-director`; il redeploy del commit remoto `f358698` e' riuscito con env pubbliche staging e service role del solo Supabase test. Il modulo resta spento: `KONA_CALL_DIRECTOR_ENABLED=false`, `KONA_CALL_DIRECTOR_STAGING_RUN=false`, nessun profilo abilitato e toggle globale `false`; l'invocazione HTTP diretta del dispatcher e' bloccata da Netlify con `403`. Il deploy non comprende ancora le modifiche locali non pubblicate. Production e' invariata. I test locali passano 72/72 per KONA e 187/187 sull'intero CRM. Le chiamate Business standard richiedono categorie esplicitamente approvate nel piano.
