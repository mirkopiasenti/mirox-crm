...
Le richieste vivono nelle tabelle server-only delle migration `065` e `066`; `067` registra le esecuzioni Codex e `068` aggiunge eventi tecnici ripuliti, segnali aggregati, outbox Telegram e checkpoint Observer. La raccolta non invia email e non inoltra log grezzi all'AI. Il cron `cron-guardian-observer` apre incidenti soltanto dopo deduplicazione e soglie, poi avvia un workflow Codex read-only sul commit interessato. Mirko e' l'unico interlocutore Telegram ammesso e puo' usare testo o vocali gia' registrati; `/salute` mostra il solo stato tecnico dell'Observer, senza dati CRM; non e' prevista conversazione audio live. Il collector telemetria esporta esplicitamente il proprio handler, così il worker può caricarlo nel bundle Netlify senza errore runtime. Nessuna patch, push o pubblicazione production puo' partire senza approvazione Telegram.
