...
| `admin-kpi-call-center.html` | KPI **Call Center**. Chiamate, appuntamenti fissati, vinti, persi, non presentati e indicatori di efficacia; viste giornaliera, settimane del mese e mensile con confronto operatrici. Solo admin |
...
| `moduli/call-center/` | **Modulo Call Center integrato** — pagine autenticate, inclusa `kona-call-director.html`, + `prenota.html` (form pubblico). Le pagine autenticate inizializzano Supabase tramite il `js/config.js` root generato dalla build, così sessioni e database restano coerenti con production/staging; il `js/config.js` interno contiene soltanto `APP_CONFIG`. `configurazione` è sotto Admin Mirox (`admin-call-center-config.html`) |
...
| `admin-kpi-call-center` | GET | **admin** | Aggrega per giorno e operatrice chiamate standard/outbound e appuntamenti, con esiti, presenze e annullamenti nell'anno selezionato |
...
| `gestisci-controllo-fissi` | POST | **admin** | Corregge un esito Fisso inserito per errore riportando una pratica da `Attivo` o `KO` a `In Attivazione`, azzerando i campi di esito e registrando l'admin nell'audit. La pagina chiede conferma prima del ripristino |
...
| `gestisci-operazioni-post-vendita` | POST | authenticated / **admin** per rimborso manuale | Gestisce tutte le scritture dei rimborsi e l'esito Apri/Chiudi: `create_rimborso_manuale` richiede `ruolo='admin'`, mentre creazione/completamento ordinari e `mark_apri_chiudi_ko` sono disponibili agli account autenticati attivi |
...
| `gestisci-anagrafiche` | GET / POST | authenticated / **admin** per eliminazione | Elenco paginato e filtrato delle anagrafiche per il reparto Post-Vendita; `action=comuni` restituisce le scelte presenti a DB, `action=comuni_istat` cerca nel catalogo ufficiale server-only e `action=export` genera un file `.xlsx` completo con gli stessi filtri multipli. `update` usa allowlist, controllo di concorrenza e validazione Comune/Provincia ISTAT; `delete` elimina solo anagrafiche senza collegamenti CRM |
...
| `guardian-telegram-webhook` | POST | webhook Telegram privato | Accetta solo il secret token configurato e il `chat_id` di Mirko; gestisce **solo testo** (i vocali non sono più trascritti: il bot chiede di scrivere il messaggio), problemi/migliorie, analisi Guardian, analisi Codex read-only, patch/test staging, proposta di rilascio e archiviazione. Ogni pulsante operativo rende automaticamente attiva la relativa richiesta per i messaggi successivi; l'archiviazione libera invece la sessione. Le azioni sensibili restano auditabili e separate |
...
| `kona-call-director-dispatcher` | cron (`*/5 * * * *`) | nessuna (cron Netlify) | Orchestratore idempotente KONA: eventi 02:00/03:30/08:00/08:30/19:10/20:00/20:05, task e conferme, piccoli batch di job, sync Google e outbox Telegram. La cancellazione Google considera conclusivi sia 404 sia 410 (evento gia' eliminato). In staging gira solo con opt-in; richiede anche il kill-switch env e il toggle globale |
| `kona-call-director-task` | POST | operatore KONA abilitato | `prossimo` / `attivo` / `esito` / `sospendi` / `riprendi`. Gli esiti passano dal motore deterministico (blacklist, esclusioni, tentativi, skip motivati); `task_id` rende idempotente anche il retry successivo a un salvataggio gia' concluso |
...
| `kona-call-director-dialog` | POST | operatore KONA abilitato | Valutazione skip "Altro" (l'IA contesta una sola volta), ricerca slot Google e CRUD appuntamenti Business con sync, calendario negozio Consumer (`negozio_slot`/`negozio_prenota`, che scrivono anche la chiamata canonica); nessuno script telefonico |
| `kona-call-director-operator` | POST | operatore KONA abilitato | Operazioni della sola interfaccia agente: lookup/creazione anagrafica Consumer, registrazione canonica, ricerca istantanea per numero, storico giornaliero, correzione esito auditata, pausa e failover AI temporaneo con notifica Telegram |
| `kona-call-director-route` | GET | authenticated | Routing leggero server-side: `kona_only`/`admin`/`abilitato`/`manual_fallback` per decidere quale interfaccia mostrare. Usato da `js/cc-header.js` per nascondere le tab manuali, reindirizzare a KONA o consentire il bypass temporaneo dopo un guasto AI |
...
| `kona-call-director-telegram-webhook` | POST | secret timing-safe + `KONA_CALL_DIRECTOR_OWNER_CHAT_ID` | Bot Telegram separato dal Guardian. Comandi rapidi (`/aiuto` `/stato` `/report` `/piano` `/approva` `/categorie` `/sospendi` `/riattiva`) e **dialogo in linguaggio naturale** interpretato da DeepSeek V4.1 Flash. Le azioni che cambiano qualcosa (sospendi, riattiva, approva piano, telefoni omaggio, direttiva) non vengono mai eseguite su interpretazione: il bot le ripropone e aspetta "si"/"no". Conversazione server-side, audit decisioni, nessun vocale |
...
- **Elementi aggiuntivi preventivo Protecta**: il `Totale sconto` è un'evidenza commerciale separata, attivabile e compilabile manualmente, mostrata in grande nella parte finale del riepilogo/PDF senza alterare i calcoli di anticipo o rata. La `Scadenza preventivo` è un testo manuale indipendente, riportato tra canone totale mensile e rimborso detrazione fiscale, con testo maggiorato.
...
- **Identità e autorizzazioni operative**: `operatore_id` e `uploaded_by` derivano sempre dal JWT autenticato. Rimborso manuale ed esito manuale Controllo L&G restano disponibili soltanto agli account `admin`. Nel riepilogo Apri/Chiudi, ogni account autenticato attivo può esitare una pratica `IN CORSO` come `KO`, tramite Netlify Function; il backend ricontrolla atomicamente lo stato prima dell'aggiornamento.
...
| `OPENAI_API_KEY` | sì (per Guardian AI) | Responses API del Guardian; solo lato server. **La trascrizione dei vocali Telegram è stata rimossa**: la env non serve più a quello |
...
| `KONA_CALL_DIRECTOR_ENABLED` | sì (per attivare KONA) | Kill-switch fail-closed: soltanto `true` abilita il modulo; assente, vuota o `false` lo mantengono spento qualunque sia la config DB |
...
| `KONA_CALL_DIRECTOR_DEEPSEEK_API_KEY` | sì (per l'assistente Telegram) | Chiave DeepSeek del dialogo in linguaggio naturale su Telegram (fallback `DEEPSEEK_API_KEY`); solo server-side, mai nel frontend |
| `KONA_CALL_DIRECTOR_DEEPSEEK_MODEL` | no | Override del modello DeepSeek (default `deepseek-flash`, cioè DeepSeek V4.1 Flash, in `kona_call_director_config.modello_deepseek`) |
...
| `KONA_CALL_DIRECTOR_CRON_SECRET` | no | Lasciare non impostata con la Scheduled Function Netlify nativa; usarla solo con uno scheduler esterno capace di inviare l'header dedicato |
...
11 pagine portate dal CC e i loro asset (`js/`, `assets/`):
...
- Chiavi permessi del CC: `registra_chiamata`, `elenco_chiamate`, `rilavorazione`, `call_center_lead_outbound`, `appuntamenti`, `prenota_interno`, `appuntamenti_oggi`, `esiti_appuntamenti`, `blacklist`. La vecchia chiave `configurazione` resta in DB per compatibilità col CC prod ma non è più usata da Mirox. Anagrafiche appartiene invece al tab Post-Vendita ed è disponibile a tutti gli account CRM attivi: tutti possono modificare dopo conferma, mentre solo gli admin possono richiedere l'eliminazione di una riga senza collegamenti storici
...
Il secondo reparto **KPI** contiene **Vendita - Consumer** ([`admin-kpi-vendita-consumer.html`](admin-kpi-vendita-consumer.html)), **Vendita - Business** ([`admin-kpi-vendita-business.html`](admin-kpi-vendita-business.html)) e **Call Center** ([`admin-kpi-call-center.html`](admin-kpi-call-center.html)). Le due pagine Vendita hanno le categorie `Mobile`, `Fisso`, `Luce & Gas`, `Allarmi`, `Assicurazioni`, le stesse tabelle, il confronto operatori e i filtri anno/punto vendita; differiscono esclusivamente per `cluster_cliente`. Il KPI Call Center offre filtri anno, mese, vista e operatrice, con viste giornaliera, settimanale e mensile. Su viewport mobili la sidebar diventa un menu richiudibile. Navigazione, stato attivo, profilo e logout sono generati da `js/admin-shell.js`; aspetto e adattamento responsive sono centralizzati in `css/admin-shell.css` e `css/admin-kpi.css`.

Nel KPI Call Center le chiamate sommano `chiamate` e `call_center_lead_outbound_chiamate` usando `data_ora` in `Europe/Rome`; una chiamata è “con risposta” quando l'esito è diverso da `non_risposto`. Gli appuntamenti fissati seguono `appuntamenti.created_at`, mentre presentati, non presentati, vinti/persi e annullati seguono la data prevista `appuntamenti.data_ora`. L'attribuzione all'operatrice usa `fissato_da_operatore_id` e consolida gli account storici tramite `profili.alias_di`; le prenotazioni pubbliche prive di operatrice restano visibili come `Online / non assegnato`. La pagina calcola inoltre tasso di risposta, appuntamento, presenza e chiusura.

Per i KPI Mobile Consumer si leggono le righe di `vendita_contratti` con `categoria_snapshot='Mobile'` e `cluster_cliente='Consumer'`, più le offerte FWA Indoor Consumer archiviate nella categoria Fisso; il mese deriva da `data_contratto` in `Europe/Rome`. Le acquisizioni sono la somma di Tied e Untied: Tied comprende FWA Indoor, Tied Call Your Country, Tied Underground e Tied Dati; Untied comprende le varianti Untied/United e Call Your Country. `Sim Convergente` e `Smart Security` sono escluse. La tabella mostra inoltre la percentuale mensile e YTD di Tied sul totale Tied + Untied. Le MNP sono separate fra opzione standard e opzione gestori selezionati; lo Smartphone usa il flag reale `dispositivo_associato` delle sole righe Mobile, quindi il modem FWA Indoor non viene contato come Smartphone. Il confronto consolida gli account storici tramite `profili.alias_di`, mentre il filtro negozio usa i codici Legnago/Cerea già presenti sui contratti.
...
Le richieste vivono nelle tabelle server-only delle migration `065` e `066`; `067` registra le esecuzioni Codex e `068` aggiunge eventi tecnici ripuliti, segnali aggregati, outbox Telegram e checkpoint Observer. La raccolta non invia email e non inoltra log grezzi all'AI. Il cron `cron-guardian-observer` apre incidenti soltanto dopo deduplicazione e soglie, poi avvia un workflow Codex read-only sul commit interessato. Un singolo errore di rete generico, incluso `Failed to fetch` durante un upload, resta in osservazione: viene aperto soltanto dopo almeno tre occorrenze o il coinvolgimento di piu' operatori e non diventa critico per il solo nome dell'operazione. I messaggi Telegram traducono il segnale in italiano semplice, spiegano diffusione e prossimo passo e non mostrano codici interni come spiegazione principale. Mirko e' l'unico interlocutore Telegram ammesso e scrive **solo testo** (i vocali non sono più trascritti); `/salute` mostra il solo stato tecnico dell'Observer, senza dati CRM. Il collector telemetria esporta esplicitamente il proprio handler, così il worker può caricarlo nel bundle Netlify senza errore runtime. Nessuna patch, push o pubblicazione production puo' partire senza approvazione Telegram.
...
Modulo che coordina la giornata dell'operatore autorizzato del Call Center: un contatto alla volta, priorita' deterministiche, arricchimento Business da fonti pubbliche, appuntamenti per giorno/zona, Google Calendar e pianificazione con Mirko via bot Telegram separato. **Stato verificato**: codice completo e Supabase test dedicato `Mirox CRM - Test KONA Call Director` (`yyorullxmdxhnunsfwwa`, `eu-west-3`) pronto con bootstrap minimo e migration `072`-`075`: 25 tabelle KONA, sei RPC, nessun dato production e zero grant browser sui nuovi oggetti. Il sito Netlify isolato [`mirox-kona-call-director-test.netlify.app`](https://mirox-kona-call-director-test.netlify.app) e' collegato alla branch `kona-call-director`. Dal 2026-08-29 il collaudo controllato e' attivo esclusivamente nello staging: `KONA_CALL_DIRECTOR_ENABLED=true`, `KONA_CALL_DIRECTOR_STAGING_RUN=true`, toggle globale `true`, un solo profilo test abilitato e modalità osservazione `true`; Google Calendar risulta collegato e sincronizzato. Il dataset sintetico `staging/004` e' stato rimosso con `database/staging/005_kona_call_director_preparazione_isabella_20260831.sql`: lo staging e' ripartito da zero contatti, chiamate, task e appuntamenti, con un solo piano approvato per lunedi' 31/08 dedicato ai contatti Consumer manuali per l'intera giornata, nessuna categoria Business e arricchimento automatico disabilitato. Gli eventuali dati reali inseriti nel collaudo restano esclusivamente nel Supabase test e vanno eliminati dopo la prova. Production e' invariata. La pagina agente mostra sempre e soltanto la shell KONA, senza tab manuali anche per gli admin; questi conservano l'accesso al sistema manuale fuori dalla schermata agente. Quando KONA e' attivo, l'operatrice lavora soltanto nell'interfaccia agente; ricerca numero, anagrafica Consumer, esiti, calendari, storico e correzioni in giornata restano interni e scrivono nelle stesse tabelle canoniche del sistema manuale. La scheda contatto mostra anche l'indirizzo di anagrafica o del lead Business. Ricontatti, non presentati, Passa in negozio e Passa a Cerea usano gli stessi filtri, ownership e stati del Call Center manuale: i tre controlli di presenza propongono prima `Presentato`/`Presentato (dimenticanza)` oppure `Ricontatta`, e solo il ricontatto apre gli esiti; un nuovo appuntamento standard usa il calendario negozio condiviso. Ogni richiesta operativa mostra un overlay bloccante fino alla risposta; gli appuntamenti Business espongono almeno dieci giornate lavorative nell'orizzonte standard, avanzano al contatto successivo dopo la sincronizzazione e sono idempotenti per task. Anche il salvataggio esito e la cancellazione di eventi Google gia' rimossi sono idempotenti, evitando falsi errori dopo un'operazione riuscita. I ricontatti Business, rilavorazione e Consumer possono essere assegnati automaticamente a KONA oppure fissati dall'operatrice indicando giorno e fascia. Le chiamate Business standard richiedono categorie esplicitamente approvate nel piano.

Pagine: `moduli/call-center/kona-call-director.html` e `admin-kona-call-director.html`. Il runbook completo di setup, collaudo, produzione e arresto e' in [`docs/KONA_CALL_DIRECTOR.md`](docs/KONA_CALL_DIRECTOR.md).

### Hardening dopo l'audit (2026-09-01)

Un audit completo del modulo ha prodotto ~40 difetti, corretti in questa stessa sessione. La migration `database/076_kona_call_director_hardening.sql` e' stata **applicata il 2026-09-14 al solo Supabase test `yyorullxmdxhnunsfwwa`** (production non toccata). I punti principali:

- **Arricchimento notturno**: i valori estratti vengono scritti solo tramite una whitelist di colonne realmente esistenti su `call_center_lead_outbound` (`telefono_extra` non e' una colonna: finiva nel patch e faceva fallire ogni arricchimento con secondo numero, con 4 retry pagati a notte); rimosso `codice_fiscale` dai campi arricchibili (non presente nello schema OpenAI, obbligava a una seconda chiamata pagata sempre inutile); i valori si applicano solo con almeno una fonte web reale oltre alla soglia di affidabilita'; l'arricchimento esclude blacklist ed esclusioni attive; con la ricerca web attiva il prompt non contiene piu' telefono, email e codice fiscale.
- **Costi**: `logUsage` non lancia piu' (un errore di scrittura non interrompe piu' il flusso ne' perde il rilascio della riserva); i token vengono contabilizzati anche su output vuoto/incompleto/non valido; il valore `0` di `max_chiamate_openai_ora`, `richieste_web_max_per_lead`, `lead_notte_obiettivo` e `soglia_lead_minime` e' rispettato come scelta esplicita (prima veniva sostituito col default e non disattivava nulla); il rate limit conta solo le chiamate con costo.
- **Motore**: claim atomico del task prima delle scritture canoniche (doppio invio non duplica piu' `chiamate`); la sorgente outbound viene chiusa sempre (niente contatto duplicato in coda); blacklist ed esclusioni lette paginate (oltre il tetto PostgREST il controllo era parziale); un errore su un singolo candidato non blocca piu' l'intera coda; orari vuoti non aprono piu' la finestra a mezzanotte; appuntamenti senza CF non generano piu' un task che blocca la coda per sempre; l'anagrafica Consumer non viene piu' azzerata sui campi facoltativi vuoti.
- **Operativita'**: "Pausa" + refresh non blocca piu' l'operatrice per 24 ore (il task sospeso viene ripreso); il piano approvato non puo' piu' essere sovrascritto dalle action `proposta`/`applica_default` ne' dalla sessione `telefoni_omaggio` forzata dal dispatcher; il failover manuale richiede un errore AI realmente registrato dal backend negli ultimi 15 minuti.
...
**Secondo giro (migration `077_kona_call_director_rpc_hardening.sql`, applicata il 2026-09-14 al solo Supabase test `yyorullxmdxhnunsfwwa`)**:
...
- Integrità località 26/08/2026: Supabase normalizza centralmente maiuscole, spazi, apostrofi ed entità HTML di `anagrafica.comune`/`provincia` per ogni sorgente, incluso il Call Center condiviso. Il catalogo `mirox_comuni_istat` contiene 7.893 denominazioni ufficiali aggiornate al 21/02/2026 ed è accessibile soltanto dal backend. La bonifica `071` ha riallineato 1.094 record deterministici, eliminato una sola anagrafica isolata e portato a zero i comuni compilati non riconosciuti; ogni variazione è conservata nell'audit server-only recuperabile. Le 119 province con sigla corrente ma in conflitto sono rimaste invariate per evitare correzioni non certe.
...
- `kona-call-director-dispatcher`: ogni **5 minuti** (Europe/Rome, DST-safe). Esegue una volta per data+evento: 02:00 arricchimento, 03:30 retention, 08:00 reminder mattina, 08:30 piano default, 19:10 report, 20:00 reminder sera e 20:05 proposta del piano successivo. Le finestre di recupero sono 20 minuti (60 per arricchimento/retention). Ogni tick gestisce task, conferme 09:00/11:30/15:30/18:00, piccoli batch di job, riconciliazione Google e outbox Telegram. **Attivo soltanto sul sito test dal 2026-08-29**, con un profilo test e modalità osservazione; production resta esclusa.
...
- La configurazione pubblica di produzione vive in `scripts/build-static.js`; per lo staging URL e publishable key arrivano dalle env `MIROX_PUBLIC_SUPABASE_*`. La build genera `dist/js/config.js`, usato anche da tutte le pagine Call Center, mentre il file sorgente `js/config.js` e' soltanto un guard contro l'apertura degli HTML non compilati
