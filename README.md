...
| `admin-kpi-vendita-consumer.html` | KPI **Vendita - Consumer**. Tab Mobile e Fisso attive, tabelle mensili e confronto operatori; filtri anno e punto vendita. Solo admin |
...
| `admin-kpi-vendita-consumer` | GET | **admin** | Aggrega i KPI Mobile e Fisso Consumer per anno e punto vendita, inclusi esiti post-vendita, attivazioni, Apri/Chiudi, tecnologie e confronto operatori |
...
Il secondo reparto **KPI** contiene **Vendita - Consumer** ([`admin-kpi-vendita-consumer.html`](admin-kpi-vendita-consumer.html)). Le categorie sono ordinate `Mobile`, `Fisso`, `Costumer Energy`, `Allarmi`, `Assicurazioni`; Mobile e Fisso sono attive, mentre le altre tab restano predisposte. Mobile mostra Acquisizioni, Dettaglio MNP e Dettaglio Smartphone. Fisso mostra Acquisizioni per tecnologia, Esiti Inserimenti, Fissi Mensili Attivati, Apri/Chiudi e mix percentuale tecnologia. Entrambe aggiornano il confronto per operatore e condividono i filtri anno/punto vendita. Su viewport mobili la sidebar diventa un menu richiudibile. Navigazione, stato attivo, profilo e logout sono generati da `js/admin-shell.js`; aspetto e adattamento responsive sono centralizzati in `css/admin-shell.css` e `css/admin-kpi.css`. La shell riusa il logo ufficiale `assets/logo.png`, la palette chiara, l'arancione, i bordi e le ombre già adottati dal resto di Mirox.
