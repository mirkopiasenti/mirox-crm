...
| `kona-call-director-dispatcher` | cron (`*/5 * * * *`) | nessuna (cron Netlify) | Orchestratore idempotente KONA: eventi 02:00/03:30/08:00/08:30/19:10/20:00/20:05, task e conferme, piccoli batch di job, sync Google e outbox Telegram. In staging gira solo con opt-in; richiede anche il kill-switch env e il toggle globale |
...
Modulo che coordina la giornata dell'operatore autorizzato del Call Center: un contatto alla volta, priorita' deterministiche, arricchimento Business da fonti pubbliche, appuntamenti per giorno/zona, Google Calendar e pianificazione con Mirko via bot Telegram separato. **Stato verificato**: codice completo e Supabase test dedicato `Mirox CRM - Test KONA Call Director` (`yyorullxmdxhnunsfwwa`, `eu-west-3`) pronto con bootstrap minimo e migration `072`-`075`: 25 tabelle KONA, sei RPC, nessun dato production e zero grant browser sui nuovi oggetti. Il sito Netlify isolato [`mirox-kona-call-director-test.netlify.app`](https://mirox-kona-call-director-test.netlify.app) e' collegato alla branch `kona-call-director`. Dal 2026-08-29 il collaudo controllato e' attivo esclusivamente nello staging: `KONA_CALL_DIRECTOR_ENABLED=true`, `KONA_CALL_DIRECTOR_STAGING_RUN=true`, toggle globale `true`, un solo profilo test abilitato e modalità osservazione `true`; Google Calendar risulta collegato e sincronizzato. Lo staging contiene inoltre il dataset sintetico ripetibile `database/staging/004_kona_call_director_dati_fittizi.sql`: 6 anagrafiche Consumer, 8 lead Business, 10 rilavorazioni e tre piani giornalieri, tutti marcati `TEST KONA`; i due appuntamenti non presentati sono record CRM sintetici e non creano eventi Google. Production e' invariata. La pagina agente mostra sempre e soltanto la shell KONA, senza tab manuali anche per gli admin; questi conservano l'accesso al sistema manuale fuori dalla schermata agente. Quando KONA e' attivo, l'operatrice lavora soltanto nell'interfaccia agente; ricerca numero, anagrafica Consumer, esiti, calendari, storico e correzioni in giornata restano interni e scrivono nelle stesse tabelle canoniche del sistema manuale. La scheda contatto mostra anche l'indirizzo di anagrafica o del lead Business. Ricontatti, non presentati, Passa in negozio e Passa a Cerea usano gli stessi filtri, ownership e stati del Call Center manuale: i tre controlli di presenza propongono prima `Presentato`/`Presentato (dimenticanza)` oppure `Ricontatta`, e solo il ricontatto apre gli esiti; un nuovo appuntamento standard usa il calendario negozio condiviso. Ogni richiesta operativa mostra un overlay bloccante fino alla risposta; gli appuntamenti Business espongono almeno dieci giornate lavorative nell'orizzonte standard, avanzano al contatto successivo dopo la sincronizzazione e sono idempotenti per task. I ricontatti Business, rilavorazione e Consumer possono essere assegnati automaticamente a KONA oppure fissati dall'operatrice indicando giorno e fascia. Le chiamate Business standard richiedono categorie esplicitamente approvate nel piano.
