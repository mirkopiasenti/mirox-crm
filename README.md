...
La dashboard espone una piccola mascotte KONA AI Guardian nell'angolo inferiore destro, affiancata da una nuvoletta di chat compatta che apre `moduli/segnala-problema.html`. La chat chiede prima di scegliere fra `Segnala un problema` e `Proponi una miglioria`, poi raccoglie un elemento alla volta con domande specifiche per il tipo scelto e conferma con un codice unico `KG-000001`. Gli operatori possono soltanto creare la richiesta e rispondere: non possono assegnare priorita', approvare analisi o avviare azioni.
