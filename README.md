...
| `admin-kpi-vendita-consumer.html` | KPI **Vendita - Consumer**. Tab Mobile, Fisso, Luce & Gas, Allarmi e Assicurazioni con tabelle mensili; filtri anno, punto vendita e operatore. Solo admin |
...
Il secondo reparto **KPI** contiene **Vendita - Consumer** ([`admin-kpi-vendita-consumer.html`](admin-kpi-vendita-consumer.html)), **Vendita - Business** ([`admin-kpi-vendita-business.html`](admin-kpi-vendita-business.html)) e **Call Center** ([`admin-kpi-call-center.html`](admin-kpi-call-center.html)). Le due pagine Vendita hanno le categorie `Mobile`, `Fisso`, `Luce & Gas`, `Allarmi`, `Assicurazioni`, le stesse tabelle e i filtri anno/punto vendita/operatore; differiscono esclusivamente per `cluster_cliente`. Il filtro operatore offre `Tutti` o un singolo profilo canonico e aggiorna immediatamente tutte le tabelle, senza un dettaglio operatori separato in fondo alla pagina. Il KPI Call Center offre filtri anno, mese, vista e operatrice, con viste giornaliera, settimanale e mensile. Su viewport mobili la sidebar diventa un menu richiudibile. Navigazione, stato attivo, profilo e logout sono generati da `js/admin-shell.js`; aspetto e adattamento responsive sono centralizzati in `css/admin-shell.css` e `css/admin-kpi.css`.
