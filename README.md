...
Guardian e' attivo sul CRM ufficiale `mirox-crm.it`, branch `main` e Supabase production `lbgwamhjkjjfwgusafbi`; l'Observer viene implementato e validato prima sullo staging `mirox-crm-staging.netlify.app` e sul Supabase separato `blwgxrszvsoqcmcmhhqr`. Le migration `067` e `068` sono state applicate e verificate sullo staging il 2026-08-11; production non e' stata modificata. Le variabili Observer sono configurate sul sito staging; il bot production `@MiroxAiGuardianBot` e il bot staging `@KonaAiGuardianBot` restano separati. Setup completo, env vars, limiti e confini sono in [`docs/KONA_AI_GUARDIAN_SETUP.md`](docs/KONA_AI_GUARDIAN_SETUP.md).
