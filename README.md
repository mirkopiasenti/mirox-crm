...
| `dashboard.html` | Home con tabs Vendita / Post-Vendita + topbar con bottoni Applicazioni / Appuntamenti Oggi / Ticket / Call Center / **Admin** (visibile solo se `ruolo='admin'`) + badge ticket aperti. L'azione flottante KONA AI Guardian nell'angolo inferiore destro apre la segnalazione problemi; il bottone Applicazioni apre un pannello espandibile sopra il saluto e collega al `Compilatore disdette` |
...
La dashboard espone ora un'azione flottante KONA AI Guardian nell'angolo inferiore destro, con mascotte robotica, che apre `moduli/segnala-problema.html`. L'operatore descrive il problema in linguaggio semplice; Guardian raccoglie un elemento alla volta e conferma con un codice `KG-000001` quando le informazioni sono sufficienti. Gli operatori possono soltanto segnalare e rispondere: non possono assegnare priorita', approvare analisi o avviare azioni.
