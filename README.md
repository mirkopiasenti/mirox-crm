...
| `admin-kpi-vendita-consumer` | GET | **admin** | Aggrega i KPI Consumer per Mobile, Fisso, Luce & Gas, Allarmi e Assicurazioni, inclusi esiti post-vendita, pagamenti, punti e confronto operatori |
...
Il secondo reparto **KPI** contiene **Vendita - Consumer** ([`admin-kpi-vendita-consumer.html`](admin-kpi-vendita-consumer.html)). Le categorie attive sono ordinate `Mobile`, `Fisso`, `Luce & Gas`, `Allarmi`, `Assicurazioni`. Mobile mostra Acquisizioni, Dettaglio MNP e Dettaglio Smartphone. Fisso mostra Acquisizioni per tecnologia, Esiti Inserimenti, Fissi Mensili Attivati, Apri/Chiudi e mix percentuale tecnologia. Luce & Gas mostra acquisiti e attivati; Allarmi mostra pezzi per modalità di pagamento e attivati; Assicurazioni mostra pezzi e punti. Tutte aggiornano il confronto per operatore e condividono i filtri anno/punto vendita. Su viewport mobili la sidebar diventa un menu richiudibile. Navigazione, stato attivo, profilo e logout sono generati da `js/admin-shell.js`; aspetto e adattamento responsive sono centralizzati in `css/admin-shell.css` e `css/admin-kpi.css`. La shell riusa il logo ufficiale `assets/logo.png`, la palette chiara, l'arancione, i bordi e le ombre già adottati dal resto di Mirox.
